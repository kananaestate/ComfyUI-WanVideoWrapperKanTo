...
# ---------------------------------------------------
# Image enhancement utilities (from the Better Motion
# Tracking Node patch, see GH issue #1408 on
# ComfyUI-WanAnimatePreprocess / #10)
# ---------------------------------------------------
...
class OnnxDetectionModelLoader:
    ...
    RETURN_NAMES = ("model", )
    FUNCTION = "loadmodel"
    CATEGORY = "WanAnimatePreprocess/KanTo"
...
class PoseAndFaceDetection:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "detection_threshold": ("FLOAT", {"default": 0.05, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "YOLO detection confidence filter. Lower = more detections (helps with occlusion), higher = fewer/more stable detections."}),
                "pose_threshold": ("FLOAT", {"default": 0.3, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "Zeroes out low-confidence keypoints in the stored pose data before drawing."}),
                "use_clahe": ("BOOLEAN", {"default": True, "tooltip": "Apply CLAHE contrast enhancement before pose detection, helps when subject/background contrast is low."}),
                "use_blur_for_pose": ("BOOLEAN", {"default": True, "tooltip": "Apply a mild Gaussian blur internally for YOLO/ViTPose inputs only, to reduce keypoint jitter. Face crops still use the sharp original frames."}),
                "blur_radius": ("INT", {"default": 5, "min": 1, "max": 15, "step": 1, "tooltip": "Radius of the Gaussian blur used for detection/pose input."}),
                "blur_sigma": ("FLOAT", {"default": 2.0, "min": 0.1, "max": 5.0, "step": 0.1, "tooltip": "Sigma of the Gaussian blur used for detection/pose input."}),
                "use_face_smoothing": ("BOOLEAN", {"default": True, "tooltip": "Motion-adaptive temporal smoothing of the face bounding box center, reduces jitter in the face crop."}),
                "face_smoothing_strength": ("FLOAT", {"default": 0.6, "min": 0.0, "max": 1.0, "step": 0.05, "tooltip": "0 = no smoothing, 1 = heavy smoothing. Smoothing automatically relaxes during fast motion."}),
                "use_constant_face_box": ("BOOLEAN", {"default": True, "tooltip": "Keep a constant pixel-size square face crop that slides to follow the face, instead of a crop that shrinks/drifts with the keypoints."}),
                "face_box_size_px": ("INT", {"default": 224, "min": 64, "max": 1024, "step": 16, "tooltip": "Pixel size of the square face crop region before it's resized to 512x512 output. Increase for close-ups, decrease for wide/full-body shots."}),
            },
            "optional": {
    ...
    def process(self, model, images, width, height,
                ...
                use_clahe, use_blur_for_pose, blur_radius, blur_sigma,
                use_face_smoothing, face_smoothing_strength,
                use_constant_face_box, face_box_size_px,
                retarget_image=None):
        detector = model["yolo"]
        ...
        # Mildly blurred copy used only for detection/pose input, this suppresses
        # high-frequency noise that otherwise makes keypoints flicker/vanish.
        # Face crops always come from the sharp, unblurred images_np below.
        ...
        if retarget_image is not None:
            ...
            if use_blur_for_pose:
                ksize = int(blur_radius) * 2 + 1
                refer_img_for_pose = cv2.GaussianBlur(refer_img.astype(np.float32), (ksize, ksize), blur_sigma)
            else:
                refer_img_for_pose = refer_img

            ref_detections = detector(
                cv2.resize(refer_img_for_pose.astype(np.float32), (640, 640)).transpose(2, 0, 1)[None],
                shape
                )[0]
            if isinstance(ref_detections, list) and len(ref_detections) > 0 and isinstance(ref_detections[0], dict):
                ref_bbox = ref_detections[0]["bbox"]
            else:
                ref_bbox = None

            if ref_bbox is None or ref_bbox[-1] <= 0 or (ref_bbox[2] - ref_bbox[0]) < 10 or (ref_bbox[3] - ref_bbox[1]) < 10:
                ref_bbox = np.array([0, 0, refer_img.shape[1], refer_img.shape[0], 1.0])

            center, scale = bbox_from_detector(ref_bbox, input_resolution, rescale=rescale)
            refer_img_crop = crop(refer_img_for_pose, center, scale, (input_resolution[0], input_resolution[1]))[0]
            refer_img_crop = preprocess_for_pose(refer_img_crop, use_clahe)
            ...
            if pose_threshold > 0.0:
                ref_keypoints[..., 2] = np.where(ref_keypoints[..., 2] < pose_threshold, 0, ref_keypoints[..., 2])
        ...
        progress = 0
        bboxes = []
        for img in tqdm(images_blurred, total=len(images_blurred), desc="Detecting bboxes"):
            detections = detector(
        ...
        kp2ds = []
        for img, bbox in tqdm(zip(images_blurred, bboxes), total=len(images_blurred), desc="Extracting keypoints"):
            if bbox is None or bbox[-1] <= 0 or (bbox[2] - bbox[0]) < 10 or (bbox[3] - bbox[1]) < 10:
                bbox = np.array([0, 0, img.shape[1], img.shape[0], 1.0])
        ...
        # --- Raw face bboxes from keypoints (pixel space, x1/x2/y1/y2 like get_face_bboxes) ---
        raw_face_bboxes = []
        for meta in pose_metas:
            face_bbox_for_image = get_face_bboxes(meta['keypoints_face'][:, :2], scale=1.3, image_shape=(H, W))
            x1, x2, y1, y2 = map(int, face_bbox_for_image)
            x1 = max(0, min(W - 1, x1))
            x2 = max(0, min(W, x2))
            y1 = max(0, min(H - 1, y1))
            y2 = max(0, min(H, y2))
        ...
        # --- Centers for motion-adaptive smoothing (rotation-robust vs. corner tracking) ---
        raw_centers = [np.array([0.5 * (x1 + x2), 0.5 * (y1 + y2)], dtype=np.float32) for (x1, x2, y1, y2) in raw_face_bboxes]
        ...
        if use_face_smoothing and len(raw_centers) > 1:
            ...
            for i in range(1, len(raw_centers)):
                ...
                # More motion between frames -> less smoothing, so fast turns aren't laggy
                dynamic_strength = base_strength * np.exp(-motion * 5.0)
                alpha = 1.0 - dynamic_strength
                smoothed_centers.append((alpha * curr + (1.0 - alpha) * prev).astype(np.float32))
        else:
            smoothed_centers = raw_centers

        # --- Final face bbox per frame, built from the smoothed center ---
        face_bboxes_xyxy = []
        if use_constant_face_box:
            half = face_box_size_px / 2.0
            for c in smoothed_centers:
                cx, cy = float(c[0]), float(c[1])
                x1 = int(np.clip(cx - half, 0, max(0, W - face_box_size_px)))
                y1 = int(np.clip(cy - half, 0, max(0, H - face_box_size_px)))
                x2 = x1 + int(face_box_size_px)
                y2 = y1 + int(face_box_size_px)
                face_bboxes_xyxy.append((x1, x2, y1, y2))
        else:
            for (x1, x2, y1, y2), c in zip(raw_face_bboxes, smoothed_centers):
                ...
                face_bboxes_xyxy.append((nx1, nx2, ny1, ny2))

        face_images = []
        face_bboxes = []
        for idx, (x1, x2, y1, y2) in enumerate(face_bboxes_xyxy):
...
class DrawViTPose:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "pose_draw_threshold": ("FLOAT", {"default": 0.3, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "Confidence threshold for whether a joint/limb is drawn. Lower it (e.g. 0.05-0.2) to keep limbs from vanishing."}),
            },
        }
    ...
    def process(self, pose_data, width, height, body_stick_width, hand_stick_width, draw_head, pose_draw_threshold=0.3, retarget_padding=64):
        ...
        # draw_aapose_by_meta_new only accepts a `threshold` kwarg on newer
        # pose_utils/human_visualization.py builds. Detect support once so this
        # node still works (just without the threshold control) on older installs
        # instead of raising a TypeError.
        try:
            supports_draw_threshold = "threshold" in inspect.signature(draw_aapose_by_meta_new).parameters
        except (TypeError, ValueError):
            supports_draw_threshold = False
        if not supports_draw_threshold:
            logging.warning(
                "DrawViTPose: installed pose_utils/human_visualization.py does not support "
                "the 'threshold' argument on draw_aapose_by_meta_new; pose_draw_threshold will be ignored."
            )
        ...
        for idx, meta in enumerate(tqdm(pose_metas, desc="Drawing pose images")):
            ...
            draw_kwargs = dict(draw_hand=draw_hand, draw_head=draw_head, body_stick_width=body_stick_width, hand_stick_width=hand_stick_width)
            if supports_draw_threshold:
                draw_kwargs["threshold"] = pose_draw_threshold
            pose_image = draw_aapose_by_meta_new(canvas, meta, **draw_kwargs)
...
NODE_CLASS_MAPPINGS = {
    "OnnxDetectionModelLoaderKanTo": OnnxDetectionModelLoader,
    "PoseAndFaceDetectionKanTo": PoseAndFaceDetection,
    "DrawViTPoseKanTo": DrawViTPose,
    "PoseRetargetPromptHelperKanTo": PoseRetargetPromptHelper,
    "PoseBoneManipulationKanTo": PoseBoneManipulation,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "OnnxDetectionModelLoaderKanTo": "ONNX Detection Model Loader (KanTo)",
    "PoseAndFaceDetectionKanTo": "Pose and Face Detection Advance (KanTo)",
    "DrawViTPoseKanTo": "Draw ViT Pose (KanTo)",
    "PoseRetargetPromptHelperKanTo": "Pose Retarget Prompt Helper (KanTo)",
    "PoseBoneManipulationKanTo": "Pose Bone Manipulation (KanTo)",
}
